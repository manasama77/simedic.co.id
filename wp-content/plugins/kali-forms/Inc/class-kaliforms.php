...
class KaliForms
{
    ...
    public function init_kaliforms()
    {
        ...
        $notifications->add_notice(
            [
                'id'      => $this->slug . '_updated_email_settings',
                'type'    => 'notice notice-info',
                'message' => $this->_get_updated_mail_settings_notice(),
            ]
        );
        /**
    ...
    /**
     * Return a notice html - remove in the later versions
     *
     * @return void
     */
    private function _get_updated_mail_settings_notice()
    {
        $html = '';
        $html .= '<h1>' . esc_html__('Kali Forms - New version info', 'kaliforms') . '</h1>';
        $html .= '<p>';
        $html .= vsprintf(
            esc_html__('Please take a minute to verify your personal %semail settings%s to make sure everything is correct. The new update brings improvements to this part and although we did our best to migrate settings without user input, we just want to make sure everything is allright.', 'kaliforms'),
            [
                '<a href="' . esc_url(admin_url('edit.php?post_type=kaliforms_forms&page=kaliforms-email-settings')) . '">',
                '</a>',
            ]
        );
        $html .= '</p>';
